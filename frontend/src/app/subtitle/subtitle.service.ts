...
export class SubtitleService {
  ...
  textToSoundText(text: string): Observable<SoundText> {
    return this.http
      .post<any>('/api/v2/phonetics', { text })
      .pipe(
        map((result) => ({
          text: result.text,
          inputUnknownIndexes: result.input_unknown_indexes,
          outputUnknownIndexes: result.output_unknown_indexes,
          outputWarningIndexes: result.output_warning_indexes,
        }))
      );
  }
